...
interface GeneratedDocumentWorkspaceProps {
  ...
  pendingIncludeReferences: boolean;
  onPendingIncludeReferencesChange: (includeReferences: boolean) => void;
...
export function GeneratedDocumentWorkspace({
  ...
  pendingIncludeReferences,
  onPendingIncludeReferencesChange,
  onUpdateSection,
  onBackToWorkbook,
  onRegenerate,
  onExportMarkdown,
  ...
  const backLabel = homeLabel ?? (isSampleMode ? "Home" : "Workbook");
  ...
  return (
    <div className="min-h-screen bg-stone-100 text-slate-900">
      <header className="print-hidden sticky top-0 z-50 border-b border-stone-200 bg-white/95 backdrop-blur">
        <div className="mx-auto max-w-7xl space-y-3 px-4 py-3">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            {homeHref ? (
              <a href={homeHref} className={cn(buttonVariants({ variant: "outline", size: "sm" }), "shrink-0 gap-2 whitespace-nowrap")}>
            ...
            <label className={cn("flex shrink-0 items-center gap-2 text-sm font-medium text-slate-600", isSampleMode && "sm:ml-auto")}>
              <span className="shrink-0">{isSampleMode ? "Sample Output Report" : "Format"}</span>
              <select
                value={pendingFormat}
            ...
            <label className="flex shrink-0 items-center gap-2 text-sm font-medium text-slate-600">
              <input
                type="checkbox"
                checked={pendingIncludeReferences}
                onChange={(event) => onPendingIncludeReferencesChange(event.target.checked)}
                className="h-4 w-4 rounded border-slate-300"
              />
              <span className="shrink-0">Verse refs</span>
            </label>
          ...
          <div className="min-w-0 overflow-hidden border-t border-stone-100 pt-2">
            ...
            <p className="mt-0.5 text-pretty text-sm leading-relaxed text-slate-500">
              {isSampleMode
                ? `Read-only sample ${documentDraft.format === "outline" ? "bullet outline" : "paragraph draft"} generated from the fixed demo answer set with ${documentDraft.options.includeReferences ? "inline verse references" : "references hidden"}.`
                : `Editable ${documentDraft.format === "outline" ? "bullet outline" : "paragraph draft"} with ${documentDraft.options.includeReferences ? "inline verse references" : "references hidden"} in the current draft.`}
            </p>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-8 lg:flex-row print-shell">
        ...
        <main className={cn("grid flex-1 gap-6", isSampleMode ? "grid-cols-1" : "xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]")}>
          {!isSampleMode && (
          ...
          <section className={cn(
            "print-preview rounded-[28px] border border-stone-200 bg-white p-6 shadow-sm",
            isSampleMode ? "min-h-[calc(100vh-10rem)]" : "xl:sticky xl:top-28 xl:h-[calc(100vh-8rem)] xl:overflow-y-auto",
          )}>
            <div className="print-hidden mb-6 flex items-center gap-2 border-b border-stone-200 pb-4">
              <Eye className="h-4 w-4 text-slate-500" />
              <div>
                <h2 className="font-semibold text-slate-900">{isSampleMode ? "Sample Preview" : "Preview"}</h2>
                <p className="text-sm text-slate-500">
                  {isSampleMode ? "This is the generated sample report rendered with the same document theme as the main app." : "This is the print/export rendering of the current draft."}
                </p>
              </div>
            </div>
            ...
            <article className="prose-shell space-y-8">
              <header className="space-y-2 border-b border-stone-200 pb-6">
                ...
                <p className="text-sm text-slate-500">Verse references: {documentDraft.options.includeReferences ? "Included" : "Hidden"}</p>
              </header>
